/**
 * Đề cương chi tiết:
 *    + Tìm hiểu react-router
 *    + Hướng dẫn sử dụng antd
 *    + Khái niệm về HOF & HOC - ứng dụng làm template
 *    + React request API (resful  API)
 *    + Middleware Redux saga, redux thunk
 *    + Cơ chế authorization token
 *    + Bài tập áp dụng
 *    + Xây dựng ứng dụng jira ứng dụng kiến thức tổng hợp
 */

// Trong khóa này sẽ tổ chức lại cấu trúc của dữ án
// Router một component sẽ đóng vai trò là một trang web của chúng ta

// Tổ chức thư mục dự án trong source này, từ nay về khóa cuối sẽ tổ chức thư mục chứ không để loạn xạ nữa

/**
 * Thư mục assets chứa các tài nguyên hình ảnh, Scss , của ứng dụng
 *
 */
